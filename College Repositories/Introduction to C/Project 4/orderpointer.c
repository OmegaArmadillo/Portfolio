...
void printProductList(struct company *companyListPointer, struct product *productListPointer)
{
		...
		for (int i = 0; i < productAmount; i++)
			{
				...
				printf ("Product Name: %s   ", (*(productListPointer+i)).Name);
				...
				for (int j = 0; j < companyAmount; j++)
					{
						...
						if ((*(productListPointer+i)).supplyId == (*(companyListPointer+j)).ID)
							{

								printf ("Company: %s   ", (*(companyListPointer+j)).Name);

							}

					}

					printf ("Price: %.2f   ", (*(productListPointer+i)).Price);
					printf ("Amount left: %d\n", (*(productListPointer+i)).Amount);
...
int askForAmount()
{
	...
	scanf ("%d", &amountOrdered);
	 		...
	 		printf ("Enter the number next toawjdklsjdkajsdnt to order or 11 to complete the order:");
...
struct company *companyListPointer;
struct product *productListPointer;
...
//setting the pointers
 ...
 companyListPointer = companyList;
 productListPointer = productList;
...
//(*productListPointer+i)
//(*companyListPointer+i)
	

            printProductList(companyListPointer, productListPointer);
	 
	 //companyListPointer++;
	 //printf("%s", (*(companyListPointer+1)).Name);
	 //	 printf("%s", (*companyListPointer).Name);
